...
\begin{algorithm}[H] 
\caption{PMX: Partially Mapped Crossover} 
\SetKwInOut{Input}{entrada} 
\SetKwInOut{Output}{salida}
...
\While{$Numerohijos<2$}{
	...
	\For{$Para cada elemento$}{ 
		...
		\If {$el lugar ocupado por j en P2 ya ha sido rellenado en la descendencia k$}{
			poner i en la posición ocupada por k en P2 \\
		}\\
		Después de haber tratado con los elementos del segmento crossover, el resto de la descendencia se puede llenar desde P2 \\
	}
}
